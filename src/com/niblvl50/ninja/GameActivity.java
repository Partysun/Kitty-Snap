...
public class GameActivity extends BaseGameActivity 
{
	...
	{  
		...
		if (!TempSettingsClass.getInstance().isSound())
			mMusic.setVolume(0f);
		this.mGameScene = new NinjaScene(setGameScreenHUD());
		return mGameScene;
	}
	...
	public void onChangeGameStatus(EventChangeGameStatus gameStatus) {
		if (gameStatus.getGameState().equals(EventChangeGameStatus.PAUSE)){
			...
			if (gameStatus.getNinja().life<=0){
				((NinjaScene) this.mGameScene).wipe();
				destroy();
	...
	public boolean onKeyDown(final int pKeyCode, final KeyEvent pEvent) {		
		if (statusScene !=1){
			...
			if (this.mEngine.getScene().hasChildScene()) {
				...
				((NinjaScene) this.mEngine.getScene()).startGeneratorObjects();
				this.mMusic.play();
			} else {		
				pause();
				}			
	...
	private void pause()
	{
		...
		((NinjaScene) this.mEngine.getScene()).stopGeneratorObjects();
		this.mMusic.pause();
	}
